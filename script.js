    ...
    input.addEventListener('input', async () => {
        ...
        try {
            ...
            const res = await fetch(
                `https://nominatim.openstreetmap.org/search?format=json&q=${input.value}&limit=5`
            );
...
async function drawRoute() {
    ...
    routeLine = L.geoJSON(best.geometry, {
        style: { color: color, weight: 8, opacity: 0.8 }
    }).addTo(map);
...
async function startCamera() {
    ...
    try {
        ...
        aiInterval = setInterval(runLowResAI, 1500);
...
async function runLowResAI() {
    ...
    if (
        top.probability > 0.97 &&
        (top.probability - second.probability) > 0.25 &&
        top.className.toLowerCase().includes("pothole")
    ) {

        detectionCounter++;

        if (detectionCounter >= 7) {
